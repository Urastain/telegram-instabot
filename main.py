...
def extract_shortcode(url):
    patterns = [
        r'/p/([A-Za-z0-9_-]+)', r'/reel/([A-Za-z0-9_-]+)', r'/tv/([A-Za-z0-9_-]+)'
    ]
...
class InstagramDownloader:
    def __init__(self):
        self.loader = instaloader.Instaloader(download_pictures=False, quiet=True)
...
async def handle_instagram_link(update: Update, context):
    ...
    if not shortcode:
        await chat.send_message("❌ Ошибка: не удалось распознать ссылку.")
        return
    ...
    try:
        ...
        if not video_url:
            ...
            response = requests.get(embed_url, headers=headers, timeout=30, verify=certifi.where())
        ...
        with open(temp_file, "rb") as video_file:
            await chat.send_video(video=video_file, caption="📥 Видео из Instagram", supports_streaming=True)
...
if __name__ == "__main__":
    ...
    telegram_app.add_handler(MessageHandler(filters.TEXT & filters.Regex(INSTAGRAM_REGEX), handle_instagram_link))
