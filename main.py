...
@app.route('/')
def home():
    return "🤖 Instagram Video Bot is running!", 200
...
def keep_alive_ping():
    """Функция для автопинга к Telegram API"""
    while True:
        try:
            requests.get(f"https://api.telegram.org/bot {TOKEN}/getMe",
                         timeout=10,
                         verify=certifi.where())
            logger.info("✅ Пинг к Telegram API: OK")
        except Exception as e:
            logger.warning(f"❌ Ошибка пинга: {e}")
        time.sleep(60)
...
class InstagramDownloader:
    ...
    def __init__(self):
        ...
        self.loader.context._session.headers.update({
            "User-Agent":
            self.loader.context.user_agent,
            "X-Requested-With":
            "XMLHttpRequest"
        })
    ...
    try:
        ...
        if not video_url:
            await status_msg.edit_text(
                "❌ Не удалось найти видео. Попробуйте позже.")
            return
        ...
        await status_msg.edit_text(
            "⚠️ Не удалось загрузить видео. Попробуйте позже.")
    finally:
        if os.path.exists(temp_file):
            os.remove(temp_file)


# Запуск автопинга и бота
if __name__ == "__main__":
    # Запускаем Flask сервер
    Thread(target=run_flask).start()
    time.sleep(3)

    # Инициализируем автопинг к Telegram API
